...
for layer in base_model.layers[:-4]: # 4 >> 6 마지막 6개의 레이어만 학습
...
model_checkpoint = ModelCheckpoint(
    filepath=os.path.join(models_dir, 'classification_VGG16_BM_4.keras'),
    save_best_only=True,
    monitor='val_loss',
    mode='min'
)
...
model.save(os.path.join(models_dir, 'classification_VGG16_4.keras'))

with open(os.path.join(models_dir, 'classification_VGG16_4_history.pkl'), 'wb') as file:
